...
const TSPWAnalysis = ({ 
  ...
  const generateTsPwReport = async (shouldStore = true) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/ts-pw/generate-tspw-report`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          mt_data: tsPwData.mt_data || [],
      ...
      if (result.success) {
        ...
        if (shouldStore) {
          const fileDescription = `Comprehensive TS-PW analysis report with insights, MT (${tsPwData.mt_data?.length || 0} records) and Value (${tsPwData.value_data?.length || 0} records) analysis for fiscal year ${fiscalInfo.current_year}`;
  ...
  const DataTable = ({ data, title, analysisType, showTsPwHighlights = true }) => {
    ...
    const tsPwColumnNames = ['TS-PW', 'TSPW', 'Product', 'Product Name'];
