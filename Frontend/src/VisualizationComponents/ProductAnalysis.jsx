...
const ProductAnalysis = ({ 
  ...
  const generateProductReport = async (shouldStore = true) => {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/product/generate-product-report`, {
        method: 'POST',
      ...
      if (result.success) {
        ...
        if (shouldStore) {
          const fileDescription = `Comprehensive product analysis report with insights, MT (${productData.mt?.data?.length || 0} records) and Value (${productData.value?.data?.length || 0} records) analysis for fiscal year ${getFiscalYearDisplay(fiscalInfo)}`;
        ...
        addMessage(`📁 Product Report ${shouldStore ? 'downloaded and stored' : 'downloaded'}: ${result.filename}`, 'success');
      } else {
