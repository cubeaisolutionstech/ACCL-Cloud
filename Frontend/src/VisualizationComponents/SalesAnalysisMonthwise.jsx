...
const SalesAnalysisMonthwise = ({ 
    ...
    try {
      ...
      const storedFileData = {
        ...
        sheets: analysisType.includes('combined') ? 
          ...
          analysisType === 'report' ?
          ['Executive Summary', 'Sales_MT_Analysis', 'Sales_Value_Analysis', 'Combined_Analysis'] :
  ...
  const generateSalesReport = async (shouldStore = true) => {
    if (!salesMtData && !salesValueData) {
    ...
    setLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/sales/generate-sales-report`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          mt_data: salesMtData?.data || [],
          value_data: salesValueData?.data || [],
          mt_columns: salesMtData?.columns || [],
          value_columns: salesValueData?.columns || [],
          fiscal_year: fiscalInfo.current_year,
          uploaded_files: {
            auditor: uploadedFiles.auditor?.filename || '',
          },
      ...
      if (result.success) {
        ...
        if (shouldStore) {
          const fileDescription = `Comprehensive sales analysis report with insights, MT (${salesMtData?.data?.length || 0} records) and Value (${salesValueData?.data?.length || 0} records) analysis for fiscal year ${fiscalInfo.current_year}`;
        ...
        addMessage(`📁 Sales Report ${shouldStore ? 'downloaded and stored' : 'downloaded'}: ${result.filename}`, 'success');
      } else {
      ...
      {canProcess && (salesMtData || salesValueData) && (
        <div className="export-merged-section">
          <div className="export-actions">
            <button
              onClick={() => generateSalesReport(false)}
              className="btn btn-primary btn-large"
              disabled={loading}
              title="Download comprehensive sales analysis report"
            >
              <Download size={16} />
              {loading ? 'Processing...' : 'Download Sales Report'}
            </button>
