...
const SavedFiles = () => {
  ...
  // Get most recent files or filter by date
  const getFilteredFiles = (files, searchCriteria) => {
    const hasDateFilter = searchCriteria.fromDate || searchCriteria.toDate;
    ...
    return sortedFiles.filter((file) => {
      ...
      const toDate = searchCriteria.toDate
        ? new Date(searchCriteria.toDate + "T23:59:59")
        : null;
    ...
    <div className="mb-4 p-4 bg-gray-50 rounded-lg border">
      <div className="flex flex-wrap items-center gap-4">
        <div className="flex items-center gap-2">
          ...
          <DatePicker
            selected={
              searchCriteria.fromDate ? new Date(searchCriteria.fromDate) : null
            }
            onChange={(date) =>
              handleSearchChange(
                type,
        ...
        <div className="flex items-center gap-2">
          ...
          <DatePicker
            selected={
              searchCriteria.toDate ? new Date(searchCriteria.toDate) : null
            }
            onChange={(date) =>
              handleSearchChange(
                type,
  ...
  const renderTable = (files, label, prefix, searchCriteria, searchType) => {
    const filteredFiles = getFilteredFiles(files, searchCriteria);
    const hasDateFilter = searchCriteria.fromDate || searchCriteria.toDate;
    const isLimitedView = !hasDateFilter && files.length > 12;
      ...
      <div className="mb-8 bg-white rounded-lg shadow-md overflow-hidden">
        <div className="bg-gray-100 px-6 py-4 border-b">
          ...
          <p className="text-sm text-gray-600 mt-1">
            {isLimitedView ? (
              <>
                Showing {filteredFiles.length} most recent files of {files.length} total files
                <span className="text-blue-600 ml-2 text-xs">
                  (Use date filter to search older files)
                </span>
              </>
            ) : (
          ...
          {filteredFiles.length === 0 ? (
            <div className="text-center py-8">
              <p className="text-gray-500">
                {files.length === 0
                  ? "No files found."
                  : "No files match the selected date range."}
              </p>
            </div>
          ) : (
